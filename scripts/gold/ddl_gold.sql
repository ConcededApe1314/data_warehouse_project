...
SELECT
    ROW_NUMBER() OVER (ORDER BY po.prd_start_dt, po.prd_sls_key) AS product_key, -- Surrogate key
...
ON
    po.prd_cat_id = pc.id
WHERE
    po.prd_end_dt IS NULL; -- Filter out all historical data, remains last date
GO
